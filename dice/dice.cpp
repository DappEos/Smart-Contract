...
#define FEE 2 
#define SINGLE_BET_MAX_PERCENT 100
  ...
  public:
    ...
    void transfer(account_name from, account_name to, eosio::asset quantity, string memo)
    {
      ...
      if (t.memo == "deposit") {
        ...
        char str_max[128];
        sprintf(str_max, "Bet amount exceeds max amount %d and %d", amt, max);
        eosio_assert(amt <= max, str_max);

        //auto trade_iter = _trades.find(sym_name);
        char str_protect[128];
        sprintf(str_protect, "Game under maintain, stay tuned %d and %d", balance.amount, trade_iter->protect);
        eosio_assert(balance.amount >= trade_iter->protect, str_protect);
