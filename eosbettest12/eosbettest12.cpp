...
class eosbettest12 : public eosio::contract {
	public:
		...
		void transfer(uint64_t sender, uint64_t receiver) {
			...
			if (transfer_data.from == _self || transfer_data.from == N(tamtamtamtam)){
				return;
			}
			...
			account_name referral = N(tamtamtamtam);
		...
		void resolvebet(const uint64_t bet_id, signature sig) {
			...
			require_auth2(N(tamtamtamtam), N(active));
			...
			uint64_t payout = 0;
			if (activebets_itr->referral != N(tamtamtamtam)){
		...
		void refundbet(const uint64_t bet_id) {
			...
			require_auth2(N(tamtamtamtam), N(random));
